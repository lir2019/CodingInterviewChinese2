...
bool LrisSymmetrical(BinaryTreeNode *pNode1, BinaryTreeNode *pNode2) {
  ...
  if (pNode1->m_nValue == pNode2->m_nValue) {
    return LrisSymmetrical(pNode1->m_pLeft, pNode2->m_pRight) &&
           LrisSymmetrical(pNode1->m_pRight, pNode2->m_pLeft);
  }
  return false;
}

bool LrisSymmetrical(BinaryTreeNode *pRoot) {
  if (pRoot == nullptr) return true;
  return LrisSymmetrical(pRoot->m_pLeft, pRoot->m_pRight);
}
...
void Test(char* testName, BinaryTreeNode* pRoot, bool expected)
{
    ...
    if(LrisSymmetrical(pRoot) == expected)
        printf("Passed.\n");
    else
        printf("FAILED.\n");
}
